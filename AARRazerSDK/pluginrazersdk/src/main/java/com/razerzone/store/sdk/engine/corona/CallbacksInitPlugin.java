...
public class CallbacksInitPlugin {
    ...
    private int mLuaStackIndexSecretApiKey = 3;
    public String mSecretApiKey = "";

	public CallbacksInitPlugin(LuaState luaState) {

		setupCallbackOnSuccess(luaState);
		setupCallbackOnFailure(luaState);

        setupSecretApiKey(luaState);
    ...
    private void setupSecretApiKey(LuaState luaState) {
        // Check if the first argument is a function.
        // Will print a stack trace if not or if no argument was given.
        try {
            luaState.checkType(mLuaStackIndexSecretApiKey, LuaType.STRING);
        }
        catch (Exception ex) {
            ex.printStackTrace();
            return;
        ...
        try {
            ...
            mSecretApiKey = luaState.checkString(mLuaStackIndexSecretApiKey);

            if (sEnableLogging) {
                Log.d(TAG, "setupSecretApiKey: secretApiKey=" + mSecretApiKey);
            }
        }
        catch (Exception ex) {
