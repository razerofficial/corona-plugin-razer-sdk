...
public class CallbacksRequestProducts {
	...
	private int mLuaStackIndexProducts = 4;
	...
	public CallbacksRequestProducts(LuaState luaState) {
	...
	private void setupProducts(LuaState luaState) {
		// Check if the first argument is a function.
		// Will print a stack trace if not or if no argument was given.
		try {
			String jsonData = luaState.checkString(mLuaStackIndexProducts);
			JSONArray jsonArray = new JSONArray(jsonData);
            List<String> products = new ArrayList<String>();
			for (int index = 0; index < jsonArray.length(); ++index) {
				String identifier = jsonArray.getString(index);
				if (null != identifier) {
					products.add(identifier);
				}
			}
            mProducts = new String[products.size()];
            mProducts = products.toArray(mProducts);
		}
		catch (Exception ex) {
